...
//    private AppSensorClient appSensorClient;
    ...
    private void initializeData() {
        String url = "http://localhost:8085/";//appSensorClient.getConfiguration().getServerConnection().getUrl();
        target = ClientBuilder.newClient().target(url);

        clientApplicationIdName = "X-Appsensor-Client-Application-Name2";//appSensorClient.getConfiguration().getServerConnection().getClientApplicationIdentificationHeaderNameOrDefault();
        clientApplicationIdValue = "myclientapp";//appSensorClient.getConfiguration().getServerConnection().getClientApplicationIdentificationHeaderValue();
    }
